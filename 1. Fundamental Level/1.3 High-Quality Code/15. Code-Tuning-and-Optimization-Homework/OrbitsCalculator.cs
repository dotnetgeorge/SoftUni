...
namespace SolarSystem
{
    class OrbitsCalculator : INotifyPropertyChanged
    {
        ...
        void OnTimerTick(object sender, EventArgs e)
        {
            ...
            var getTime = (now - _startTime).TotalMilliseconds;
            var reverseTime = ReverseTime ? -1 : 1;

            Days += getTime * DaysPerSecond / 1000.0 * reverseTime;
            _startTime = now;
            Update("Days");
            OnTimeChanged();
        }
        ...
        private void EarthRotation()
        {
			...
			{
                var calc = step * Days;
				EarthRotationAngle = calc / EarthRotationPeriod;
			}
